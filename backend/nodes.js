...
function updateMeasurements() {
  ...
  var latestMeasurement = date
    .subtract(date.minutes() % 5, "minutes")
    .format("YYYY-MM-DD HH:mm");
  // map: node.id => node
  var nodeData = _.mapKeys(commonNodes, (node) => node.id);
  var nodeNames = _(commonNodes)
    .map((node) => `('${node.id}')`)
    .join();
  var query =
    "select n.node_id as node_id, d.day as date, coalesce(m.status, 0) as status " +
    "from (select generate_series(timestamp '" +
    latestMeasurement +
    "', timestamp '" +
    latestMeasurement +
    "' - interval '" +
    (measurementsCount * 5 - 1) +
    " minute', interval '-5 minute')) d(day) " +
    "cross join (select * from (values " +
    nodeNames +
    ") as node_names) as n(node_id) " +
    "left join node_measurements m ON n.node_id = m.node_id and m.date = d.day order by d.day desc;";

  var response = {};

  postgres.sequelize
    .query(query, { model: postgres.NodeMeasurement })
    .then((measurements) => {
        ...
        .then((stats) => {
          for (let stat of stats) {
            ...
            if (stat.all_24h >= requiredMeasurements24h) {
              response[stat.node_id].uptime_24h =
                Math.round((stat.up_24h / stat.all_24h) * 10000) / 100;
            }

            if (stat.all_30d >= requiredMeasurements30d) {
              response[stat.node_id].uptime_30d =
                Math.round((stat.up_30d / stat.all_30d) * 10000) / 100;
            }
          }
        });
    })
...
postgres.sequelize.addHook("afterBulkSync", () => {
  if (process.env.UPDATE_DATA == "true") {
    ...
    setInterval(updateMeasurements, 60 * 1000);
  }
  updateMeasurements();
});
