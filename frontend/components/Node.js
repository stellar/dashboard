...
export default class Node extends React.Component {
  ...
  updateSize() {
    ...
    if (window.innerWidth < 1150 && !this.state.small) {
      ...
      (this.state.small || this.state.chartWidth != panelWidth / 3)
    ) {
      this.setState({
        small: false,
        expanded: false,
        chartWidth: panelWidth / 3,
      });
    }
  }
          ...
          <div className="node-panel-large">
            ...
            {this.state.loading ? null : (
              <div className="mui-col-md-2 mui--hidden-sm mui--hidden-xs">
                ...
                <div className="afterI">
                  24H{" "}
                  <span className={style_24h}>
                    {this.state.uptime_24h === undefined
                      ? "n/d"
                      : `${this.state.uptime_24h}%`}
                  </span>
                  &nbsp; 30D{" "}
                  <span className={style_30d}>
                    {this.state.uptime_30d === undefined
                      ? "n/d"
                      : `${this.state.uptime_30d}%`}
                  </span>
                  &nbsp;
                </div>
              </div>
            ...
            {this.state.loading ? (
              ...
              <div
                className={
                  "mui-col-md-4 mui-col-sm-1 mui-col-xs-3 node-barchart-container"
                }
              >
                {this.state.small ? (
                  ""
                ...
                {this.state.small ? (
                  <i
                    className="material-icons node-dropdown-button"
                    onClick={() =>
          ...
          {this.state.expanded && this.state.small ? (
            <div className="node-dropdown">
              ...
              <div className="small gray margin-top10 node-update-interval-container">
                Checked every 5 mins. Last: {this.props.uptime.latest[0].date}{" "}
                UTC
              </div>
            </div>
          ) : (
